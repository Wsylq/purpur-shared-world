 ...
 * applyPush() also schedules a chunk reload on the main thread so the
 * in-memory LevelChunk is refreshed immediately (FIX #1/#3).
 *
...
public class RemoteDataClient {
    ...
    public static RemoteDataClient get() {
        return INSTANCE;
    }
        ...
        readerThread = new Thread(() -> {
            while (running && connected) {
                try {
                    ...
                    // Unsolicited server-push packets have requestId == 0
                    if (pkt.requestId == 0) {
     ...
     * CHUNK_PUSH: master is telling us another server saved new chunk data.
     *
     * FIX #1/#3: applyPush now properly schedules a main-thread chunk reload.
     */
    private void handleServerPush(RemoteDataPacket pkt) {
        if (pkt.opCode == RemoteDataPacket.OpCode.CHUNK_PUSH) {
            ...
            // FIX #1/#3: Schedule chunk reload on main thread via applyPush
            try {
        ...
        while (running) {
            if (!connected) {
                try {
                    ...
                    LOGGER.warning("[RemoteData] Could not connect to master (" + e.getMessage()
